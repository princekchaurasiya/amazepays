    ...
    public function createOrder(Request $request)
    {
        try {
            ...
            if ($payment && ($payment->billing_email || $payment->billing_tel || $payment->billing_name)) {
                $qsOrder->email        = $payment->billing_email ?? null;
                $qsOrder->mobile       = $payment->billing_tel ?? null;
            ...
            try {
                if ($isSuccess) {
                    ...
                    $woohoo->handleSuccessFullOrder($handlerData);
                    
                    // Woohoo order created successfully - update to COMPLETE
                    $qsOrder->refresh(); // Reload to get latest data from handleSuccessFullOrder
                    ...
                    Log::info("✅ QsOrder status updated to COMPLETE (Woohoo order created successfully)", [
                        'order_id' => $qsOrder->id,
                        'woohoo_order_id' => $qsOrder->woohoo_order_id
                    ]);
                } else {
                    ...
                    Log::error("❌ QsOrder status updated to FAILED (Woohoo order creation failed)", [
                        'order_id' => $qsOrder->id,
