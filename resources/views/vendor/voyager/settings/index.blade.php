...
@section('css')
    <style>
        ...
        /* Overview tab table */
        .settings-overview-table th,
        .settings-overview-table td {
            font-size: 12px;
            vertical-align: middle !important;
        }
        .settings-overview-table code {
            font-size: 11px;
        }
    </style>
@stop
            ...
            <div class="panel">
                ...
                <div class="page-content settings container-fluid">
                    <ul class="nav nav-tabs">
                        ...
                        {{-- Read-only overview tab (does not change any existing behavior) --}}
                        <li>
                            <a data-toggle="tab" href="#settings-overview">
                    ...
                    <div class="tab-content">
                        ...
                        <div id="settings-overview" class="tab-pane fade in">
                            @php
                                $allSettings = collect($settings)->flatMap(function ($groupSettings, $group) {
                                    return collect($groupSettings)->map(function ($setting) use ($group) {
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped settings-overview-table">
                                    <thead>
                                        <tr>
                                            <th>Group</th>
                                            <th>Key</th>
                                    ...
                                    <tbody>
                                        @foreach($allSettings as $setting)
                                            @php
                                                $isSensitive = Str::contains($setting->key, ['password', 'secret', 'token']);
                                                $valueDisplay = $isSensitive
                                                    ? '********'
                                                    : \Illuminate\Support\Str::limit($setting->value, 60);

                                                $details = $setting->details ? json_decode($setting->details, true) : [];
                                                $updatedAt = $details['update_time'] ?? null;

                                                $expiresAt = null;
                                                if ($setting->key === 'api.bearer_token' && !empty($setting->value)) {
                                                    try {
                                                ...
                                                if ($expiresAt) {
                                                    try {
                                                        ...
                                                        $expiry = \Carbon\Carbon::parse($expiresAt, 'Asia/Kolkata');
    ...
    <script>
        $('document').ready(function () {
            ...
            $('[data-toggle=\"tab\"]').click(function() {
                // Preserve existing behavior for real group tabs only
                var label = $(this).text().trim();
                if (label !== 'Overview') {
                    $('.setting_tab').val(label);
                }
            });
