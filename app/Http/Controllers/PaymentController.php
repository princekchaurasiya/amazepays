...
*/
    ...
    public function responseCcavenue(Request $request)
    {
        ...
        if ($order_status === 'Success') {
            ...
            // dd($new_data);
            $user_id = QsOrder::where('order_id', $data[0]['order_id'])->first();
            // dd($user_id);
            Auth::loginUsingId($user_id['user_id']);
            DB::table('cc_avenue_payment')->insert(json_encode($new_data));
            // CcAvenuePayment::create($new_data);
            // CcAvenuePayment::updateOrInsert($new_data);
