    ...
    <div class="mx-auto max-w-7xl px-4 py-8">
        <nav class="mb-6 text-sm text-gray-500" aria-label="Breadcrumb">
            <ol class="flex flex-wrap items-center gap-2">
                ...
                <li class="font-medium text-gray-900">{{ $category->name }}</li>
            </ol>
        </nav>

        <h1 class="text-center text-2xl font-bold tracking-tight text-gray-900 md:text-3xl">{{ $category->name }}</h1>
        @if(isset($products) && $products->isNotEmpty())
            <p class="mt-2 text-center text-sm text-gray-500">{{ __('storefront.brands_count', ['count' => $products->count()]) }}</p>
        @endif
        ...
        @if (!empty($allCategories) && $allCategories->isNotEmpty())
            <div class="mt-8 flex gap-2 overflow-x-auto pb-2 md:flex-wrap md:justify-center">
                @foreach ($allCategories as $singleCategory)
                    @php $active = $singleCategory->id === $category->id; @endphp
        ...
        <div class="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            @forelse ($products ?? [] as $product)
                ...
                @endif
            @empty
                <p class="col-span-full py-12 text-center text-gray-500">{{ __('storefront.empty_category') }}</p>
            @endforelse
        </div>
    </div>
@endsection
